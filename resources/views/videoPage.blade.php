            ...
            <div class="row">
                @if (isset($movie))
                    <video id='video' width="100%" height="450px" controls controlsList="nodownload" preload="none" crossOrigin="anonymous" style="border:2px solid; color:#FF6347">
                        <source src="http://192.168.10.10:3000/stream/{{ $movie->hash }}" type="video/mp4">
                          @if (Auth::user()->language == 'fr')
                              <track src="http://192.168.10.10:3000/subtitles/{{ $movie->imdb }}/fr/{{ $movie->ses }}/{{ $movie->ep }}" kind="subtitles" srclang="fr" label="French" default>
                          @else
                            <track src="http://192.168.10.10:3000/subtitles/{{ $movie->imdb }}/fr/{{ $movie->ses }}/{{ $movie->ep }}" kind="subtitles" srclang="fr" label="French">
                        @endif

                          <track src="http://192.168.10.10:3000/subtitles/{{ $movie->imdb }}/en/{{ $movie->ses }}/{{ $movie->ep }}" kind="subtitles" srclang="en" label="English">
